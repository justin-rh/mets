...
- **Threading priority**: (1) stored Message-IDs vs In-Reply-To/References ([threading](https://www.mailersend.com/blog/email-threading)); (2) `[T-1000042]` subject token fallback (Salesforce-style — [E2C threading](https://help.salesforce.com/s/articleView?id=service.support_email_to_case_threading.htm)); (3) fuzzy match conservative or disabled — false merges worse than duplicates.
